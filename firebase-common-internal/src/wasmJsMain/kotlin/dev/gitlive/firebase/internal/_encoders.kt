...
public actual fun FirebaseEncoder.structureEncoder(descriptor: SerialDescriptor): FirebaseCompositeEncoder = when (descriptor.kind) {
    ...
    StructureKind.MAP -> {
        ...
        FirebaseCompositeEncoder(settings) { _, index, value ->
            if (index % 2 == 0) {
                lastKey = (value as? String) ?: Json.encodeToString(value)
            } else {
                map[lastKey] = value
            }
        }
...
private fun FirebaseEncoder.encodeAsMap(descriptor: SerialDescriptor): FirebaseCompositeEncoder = mutableMapOf<String, Any?>()
    .also { value = it }
    .let {
